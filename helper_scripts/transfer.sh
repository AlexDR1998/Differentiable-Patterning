...
tar --exclude="${SOURCE_FOLDER}models" --exclude="${SOURCE_FOLDER}logs" --exclude="${SOURCE_FOLDER}output" --exclude="${SOURCE_FOLDER}.git" -cf "$TEMP_TAR_FILE" -C "$SOURCE_FOLDER" .
...
# kubectl -n $namespace cp "$SOURCE_FOLDER" "$pod_name":"$PVC_MOUNT_PATH"
