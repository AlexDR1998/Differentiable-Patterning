...
from Common.dataloader.adhesion_mask import adhesion_mask_convex_hull, adhesion_mask_convex_hull_circle, adhesion_mask_convex_hull_ellipse
...
def load_micropattern_nodal_lefty_cer(
      ...
      PROCESSING_MODES=["mean_0_std_1","saturate","map_to_0_1","align","pad_to_full_width","downsample"],
      EXP_MODES=[1]
      ):
...
def show_histograms(data,channel_names,title="Pre processing histograms"):
    ...
    for t in range(n_timesteps):
        axes[t].hist(rearrange(data[t],"B X Y C -> (B X Y) C"), bins=50, alpha=1.0, label=channel_names,density=True)
...
def process_data(
    ...
    def histogram_equalise(data):
        ...
        if HIST_BINS is None:
            for channel in range(data[0].shape[-1]):
                ...
                channel_percentiles.append(np.percentile(a=channel_data_flat, q=HIST_EQS))
        else:
    ...
    def batch_average(arr):
        ...
        """
        if BATCH_AVERAGE:
            return [reduce(timestep,"BATCH X Y C -> () X Y C","mean") for timestep in arr]
        else:
            return arr
    ...
    def _pad_to_full_width(arr):
        ...
        arr = jnp.pad(arr,((0,0),(floor(X_pad),ceil(X_pad)),(floor(Y_pad),ceil(Y_pad)),(0,0)),mode="edge")
        return arr
    ...
    def _remove_background(arr):
        ...
        for b in tqdm(range(arr.shape[0])):
            _b = []
            for c in range(arr.shape[-1]):
                background = restoration.rolling_ball(arr[b,:,:,c], radius=BACKGROUND_RADIUS)
            ...
            for c in range(timestep.shape[-1]):
                for b in range(timestep.shape[0]):
                    ...
                    timestep = timestep.at[b,:,:,c].set(
                        jnp.where(timestep[b,:,:,c] > thresh, timestep[b,:,:,c], 0.0)
                    )
    ...
    return data, {"backgrounds":backgrounds,"foregrounds":foregrounds,"HIST_BINS":HIST_BINS}
...
def load_micropattern_circle_8ch(
    ...
    """
    impath_sftl = PVC_PATH+"Data/Timecourse 60h June/S2 FOXA2_SOX17_TBXT_LMBR/Max Projections/*"     # Sox17, Foxa2, TbxT, Lmbr
    impath_dcln = PVC_PATH+"Data/Nodal_LEFTY_CER/**"                                                  # Lmbr, Cer Lefty, Nodal
    impath_lls = PVC_PATH+"Data/Timecourse 60h June/Smad23_LEF 48h/Max Projections/*"            # Lef1, Lmbr, Smad23
    ...
    data_lls = data_lls[:,:,:,:,:1] # Also remove smad23 channel as guillaume recommended
...
def pad_to_biggest(ims):
    ...
    for im in ims:
        ...
        padded_im = jnp.pad(im, ((pad_top, pad_bottom), (pad_left, pad_right),(0, 0)), mode="constant")
...
def align_centre_of_mass(img_stack):
    ...
    for i in range(N):
        ...
        greyscale = ndi.gaussian_filter(greyscale, sigma=4.0)  # Smooth the image to reduce noise
        ...
        labeled_foreground = skimage.morphology.convex_hull_image(labeled_foreground,tolerance=0.5)
        footprint = skimage.morphology.disk(32)
        labeled_foreground = skimage.morphology.binary_erosion(labeled_foreground, footprint=footprint)
