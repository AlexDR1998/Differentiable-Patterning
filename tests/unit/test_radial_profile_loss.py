...
def radial_profile_grouped(prediction, target, mask, boundary=None, radial_bins=16):
    return radial_profile_grouped_loss(
        prediction,
        target,
        where=mask,
        aux={"spatial_mask": boundary, "radial_bins": radial_bins},
    )
...
def test_matching_radial_profiles_have_zero_loss():
    prediction = _central_prediction()
    target = _target_layout(prediction)
    channel_mask = jnp.ones((1, 12, 1, 1), dtype=bool)

    loss = radial_profile_grouped(prediction, target, channel_mask, radial_bins=4)
...
def test_radial_loss_has_finite_nonzero_gradients():
    prediction = _central_prediction()
    target = _target_layout(prediction).at[:, 3].set(0.5)
    channel_mask = jnp.ones((1, 12, 1, 1), dtype=bool)

    def loss_fn(values):
        return radial_profile_grouped(values, target, channel_mask, radial_bins=4).sum()
...
def test_duplicate_measurements_share_one_channel_weight():
    RADIAL_CHANNEL_WEIGHTS = jnp.asarray(
        MICROPATTERN_GROUPED_12CH_SCHEMA.measurement_weights
    )
