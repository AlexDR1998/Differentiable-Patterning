...
job_id = "gnca_contiguous"
sbatch_log_dir = "logs"

for CONTIGUOUS_REGULARISER in [1.0]:
    # Create log directory if it doesn't exist
    os.makedirs(sbatch_log_dir, exist_ok=True)
    command = [
        "sbatch",
        f"--job-name={job_id}_{CONTIGUOUS_REGULARISER}",
        f"--output={sbatch_log_dir}/{job_id}_{CONTIGUOUS_REGULARISER}.out",
        "launch.sh",
        str(CONTIGUOUS_REGULARISER)
    ]
