...
from Experiments.config_helpers import build_registry_tags, build_wandb_tags
from NCA.model.factory import PORTABLE_MODEL_FAMILIES


BUNDLE_SCHEMA_VERSION = 3
DEFAULT_MODEL_FACTORY = "NCA.model.factory:build_model"
CONFIG_ID_LENGTH = 12
...
def open_model_bundle(path: Union[str, Path]) -> ModelBundle:
    ...
    return ModelBundle(bundle_path, manifest, experiment_config_from_mapping(config_value))
    ...
    try:
        ...
        manifest = {
            ...
            "data": {
                ...
                "task": (
                    config["data"].get(DATA_SECTIONS.get(config["data"]["dataset"])) or {}
                ).get("task"),
            },
            "provenance": {
            ...
            for path in self.bundle_paths():
                ...
                connection.execute(
                    ...
                    (
                        ...
                        manifest.data.get("task"), bundle.config.seed,
                ...
                for tag in build_registry_tags(bundle.config):
                    connection.execute(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Inspect and rebuild the local model catalogue."
    )
    parser.add_argument(
        "--root",
        type=Path,
