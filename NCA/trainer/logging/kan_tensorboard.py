...
class kaNCA_Train_log(NCA_Train_log):
	...
	def _rollout_states_from_log_dict(self,log_dict,max_states=4):
		if "states" not in log_dict:
			return None
		rollout_states = log_dict["states"]
		if isinstance(rollout_states, (list, tuple)):
			batches = [jnp.asarray(batch) for batch in rollout_states if batch is not None]
			if not batches:
				return None
			states = jnp.concatenate(batches, axis=0)
		else:
			states = jnp.asarray(rollout_states)
			if states.ndim == 3:
				states = states[None]
		if states.ndim != 4:
			return None
