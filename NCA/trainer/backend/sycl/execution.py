...
class SyclTwoTileExecution(TrainingExecution):
    ...
    def transform_loss(self, compute_loss):
        ...
        def tile_local_loss(nca_diff, nca_static, x, y, key, loss_weights):
            ...
            return mean_loss, (
                self._add_local_tile_axis(states, "state"),
    ...
    def _prepare_boundary_specs(self):
        ...
        tile_callbacks = self._split_between_tiles(
            self.trainer.boundary_callbacks, "boundary callbacks"
        )
        specs = []
    ...
    def prepare_inputs(self, nca, x, y, opt_state, key):
        ...
        for name, values in (
            ("boundary callbacks", self.trainer.boundary_callbacks),
    ...
    def prepare_log_dict(self, log_dict):
        ...
        result["states"] = self._unpack_slots(result["states"])
