...
class NCA_Trainer(object):
	...
	def _run_nca_steps(
		...
		def nca_step(carry, j):
			...
			key_array = list(jr.randint(
				step_key,
				shape=(self.BATCHES, state_shape, 2),
				minval=0,
	...
	def train(self,
		...
		configured_loss_names = (
			(LOSS_FUNC_STR,) if isinstance(LOSS_FUNC_STR, str) else tuple(LOSS_FUNC_STR)
		)
		is_multi_target = configured_loss_names == ("multi_target",)
		...
		def make_step(nca,x_latent,y_proc,t,opt_state,key):
			...
			def compute_loss(nca_diff,nca_static,x_latent,y_proc,t,key):
				...
				if is_multi_target:
					...
					prediction = jnp.stack(x_proc)[:, :, :self.OBS_CHANNELS]
					target = jnp.stack(y_proc)[:, :, :self.DATA_CHANNELS]
					...
					loss_key = list(jr.split(key, self.BATCHES))
					y_latent = vv_real_to_latent(y_proc)
					losses = jnp.asarray(jtu.tree_map(
						self.loss_func,
						x_proc,
						y_proc,
						x_latent,
		...
		if loss_initialiser is not None:
			...
			y_decoded_list = y_decoded_obs
			mask_list = self.LOSS_TIME_CHANNEL_MASK
